...
const MerchItemDetail = () => {
    ...
    const fetchItemDetails = async () => {
      try {
        ...
        if (isLoggedin && userData) {
          setIsInWishlist(userData.wishlist?.includes(itemId) || 
            itemData.wishlistedBy?.includes(userData._id));
        }
        
        // Fetch items from the same artist
        if (itemData.artist && itemData.artist._id) {
          const artistRes = await fetch(`${backendUrl}/api/merch/artist/${itemData.artist._id}`, {
            credentials: 'include'
          });
          ...
          if (artistRes.ok) {
            ...
            const otherArtistItems = artistItemsData.filter(item => item._id !== itemId);
        ...
        if (allItemsRes.ok) {
          ...
          const relatedByType = allItems.filter(
            relItem => relItem.type === itemData.type && 
            relItem._id !== itemId &&
  ...
  const renderItemCard = (item) => (
    <div className="related-item-card" onClick={() => navigate(`/merch/${item._id}`)}>
        ...
        <div className="item-detail-content">
            ...
            {item.images.length > 1 && (
              <div className="thumbnail-gallery">
                {item.images.map((image, index) => (
                  ...
                  >
                    <img src={`${backendUrl}${image}`} alt={`${item.name} view ${index + 1}`} />
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="item-info">
            ...
            <div className="item-availability">
              <span>Availability:</span> {item.stock > 0 ? `${item.stock} in stock` : 'Out of stock'}
            </div>
            ...
            <div className="purchase-controls">
              <div className="quantity-control">
                ...
                <span>{quantity}</span>
                <button 
                  onClick={() => setQuantity(Math.min(item.stock || 10, quantity + 1))}
